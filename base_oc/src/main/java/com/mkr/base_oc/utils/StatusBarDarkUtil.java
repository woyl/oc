...
public class StatusBarDarkUtil {
     ...
     */
    public static boolean setStatusBarMode(Activity activity, boolean isDark) {
        // 无语系列：ZTK C2016只能时间和电池图标变色。。。。
        if (StatusBarDeviceHelper.isZTKC2016()) {
            return false;
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isMIUICustomStatusBarLightModeImpl() && MIUISetStatusBarLightMode(activity.getWindow(), isDark)) {
                return true;
            } else if (FlymeSetStatusBarLightMode(activity.getWindow(), isDark)) {
                return true;
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                Android6SetStatusBarLightMode(activity.getWindow(), isDark);
                return true;
            }
        }
        return false;
    ...
    private static boolean isMIUICustomStatusBarLightModeImpl() {
        return StatusBarDeviceHelper.isMIUIV5() || StatusBarDeviceHelper.isMIUIV6() ||
                StatusBarDeviceHelper.isMIUIV7() || StatusBarDeviceHelper.isMIUIV8();
    }
}
